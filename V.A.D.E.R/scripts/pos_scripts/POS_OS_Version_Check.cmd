...
set "report=%~dp0\..\..\POS_OS_Version_Report.txt"
del %report%

cls

echo ************** POS OS Version Check *************
...
echo This script will return the Windows version (Win7/Win10) of POS ONLY. This will not work on kiosks.
echo.
...
echo POS OS version report generated on %datestamp% at %timestamp% ^| > "%report%"
echo ---------------------------------------------------------- >> "%report%"
echo. >> "%report%"
echo. >> "%report%"
...
for /f %%i in (%terminals%) do (
    ...
    (
        for /f "tokens=2 delims==" %%V in ('psexec \\%%i /accepteula /u admin /p Redacted wmic os get Version /value 2^>nul ^| find "Version"') do (
            set "Ver=%%V"
            if "!Ver:~0,2!" equ "6." (
